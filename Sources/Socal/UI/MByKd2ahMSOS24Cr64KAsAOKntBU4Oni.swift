...
class MByKd2ahMSOS24Cr64KAsAOKntBU4Oni: NSObject {
    ...
    func hP9nZOycmC2VGc6GRkbC1pwPSjQdCpMi(reloadAll: Bool = false, ignoreErrors: Bool = false) {
        ...
        case .web:
            ...
            dispatchGroup.enter()
            let task = ighttpWeb.json(.init(TkRKqjykgs2HAKe4qgpkeH5hxOUor0gV: .getUserPosts(username: Snehtulthenrstkrsenrstenr.igUserName, nextMaxID: nextMaxID, userID: "6517968804"/*Snehtulthenrstkrsenrstenr.gsaZ86kkBusFQABHgjTVF1BjErFeXNwM8*/))) { (result: Result<MFFqPUT5YhDvOqeYK45rnzUkrqhlIYgK, NetworkingError>) in
